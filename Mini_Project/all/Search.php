    ...
    if (isset($_GET['search'])) {
        ...
        for($i = 0; $i < count($search1); $i++) {
            if($search!="" AND $search!=" "){
                ...
                INNER JOIN video ON dtlolahraga.idDtlOlahraga = video.idDtlOlahraga
                INNER JOIN instruktur ON video.idInstruktur = instruktur.idInstruktur
                WHERE olahraga.tipeOlahraga LIKE '%$search1[$i]%' 
                OR dtlOlahraga.namaOlahraga LIKE '%$search1[$i]%' 
                OR video.judulVideo LIKE '%$search1[$i]%'
                OR video.levels LIKE '%$search1[$i]%'
                OR instruktur.namaInstruktur LIKE '%$search1[$i]%'";
            }
        }
    } else {
        if ($_GET['nama'] == null) {
